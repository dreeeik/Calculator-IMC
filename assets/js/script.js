...
function getNivelImc (imc) {
    ...
    if (imc >= 39.9) return nivel [5]; if(imc >= 34.9)return nivel [4]; 
    if(imc >= 29.9)return nivel [3]; if(imc >= 24.9)return nivel [2];
    if(imc >= 18.5)return nivel [1]; if(imc < 18.5)return nivel [0];   
}
...
function setResultado (msg, isValid){
    ...
    p.innerHTML = msg;
    resultado.appendChild(p, getImc);
}
